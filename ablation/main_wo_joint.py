...
from ablation.decoder_wo_joint import Decoder
...
indices_decoder_t = torch.tensor([0, 2]).to(cfg.device)
...
class IWNet(nn.Module):
    ...
    def fit(self, log_dir=False, batch_size=cfg.batch_size, lr=float(cfg.lr), epochs=cfg.epochs):
        if not log_dir:
            log_dir = f'ablation_exp/wo_joint/{(datetime.now().strftime("%Y.%m.%d-%H.%M.%S"))}'
