...
class IWNet(nn.Module):
    ...
    def fit(self, log_dir=False, batch_size=cfg.batch_size, lr=float(cfg.lr), epochs=cfg.epochs):
        if not log_dir:
            log_dir = f'ablation_exp/wo_freq/{(datetime.now().strftime("%Y.%m.%d-%H.%M.%S"))}'
        ...
        for epoch in range(1, epochs + 1):
            ...
            for step, (cover_images, mask) in enumerate(iterator):
                ...
                extract_wm_all = decode(u_embedded_attack_type_all, self.decoder_t)
                extract_wm_commom = decode(u_embedded_attack_type_common, self.decoder_d)
                extract_wm_df = decode(u_embedded_attack_type_df, self.decoder_d)
                ...
                loss_total = loss_encoder * cfg.encoder_w + loss_noise_all * cfg.all_w + loss_nosie_common * cfg.common_w + loss_gnn * cfg.gnn
            ...
            with torch.no_grad():
                for step, (images, mask) in enumerate(iterator):
                    ...
                    val_jpeg_wm_all = validation_attack(input, u_embedded, jpeg, self.decoder_t)
                    val_jpeg_wm_common = validation_attack(input, u_embedded, jpeg, self.decoder_d)

                    val_resize_wm_all = validation_attack(input, u_embedded, resize, self.decoder_t)
                    val_resize_wm_common = validation_attack(input, u_embedded, resize, self.decoder_d)

                    val_medianblur_wm_all = validation_attack(input, u_embedded, medianblur, self.decoder_t)
                    val_medianblur_wm_common = validation_attack(input, u_embedded, medianblur, self.decoder_d)

                    val_gaublur_wm_all = validation_attack(input, u_embedded, gau_blur, self.decoder_t)
                    val_gaublur_wm_common = validation_attack(input, u_embedded, gau_blur, self.decoder_d)

                    val_gauNoise_wm_all = validation_attack(input, u_embedded, gau_noise, self.decoder_t)
                    val_gauNoise_wm_common = validation_attack(input, u_embedded, gau_noise, self.decoder_d)

                    val_dropout_wm_all = validation_attack(input, u_embedded, dropout_noise, self.decoder_t)
                    val_dropout_wm_common = validation_attack(input, u_embedded, dropout_noise, self.decoder_d)

                    val_saltPepper_wm_all = validation_attack(input, u_embedded, salt_pepper_noise, self.decoder_t)
                    val_saltPepper_wm_common = validation_attack(input, u_embedded, salt_pepper_noise, self.decoder_d)

                    val_identity_wm_all = validation_attack(input, u_embedded, identity, self.decoder_t)
                    val_identity_wm_common = validation_attack(input, u_embedded, identity, self.decoder_d)

                    val_simswap_wm_all = validation_attack(input, u_embedded, simswap, self.decoder_t, type="all")
                    val_simswap_wm_df = validation_attack(input, u_embedded, simswap, self.decoder_d, type="deepfake")

                    val_ganimation_wm_all = validation_attack(input, u_embedded, ganimation, self.decoder_t, type="all")
                    val_ganimation_wm_df = validation_attack(input, u_embedded, ganimation, self.decoder_d, type="deepfake")

                    val_stargan_wm_all = validation_attack(input, u_embedded, stargan, self.decoder_t, type="all")
                    val_stargan_wm_df = validation_attack(input, u_embedded, stargan, self.decoder_d, type="deepfake")
