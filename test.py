...
class IWNet(nn.Module):
    def __init__(self):
        super().__init__()
        self.encoder = Encoder(num_dirs=_num_enc_dirs).to(cfg.device)
    ...
    def fit(self, log_dir=False, batch_size=cfg.batch_size, lr=float(cfg.lr), epochs=cfg.epochs):
        if not log_dir:
            log_dir = f'exp_highpass_test/{(datetime.now().strftime("%Y.%m.%d-%H.%M.%S"))}'
        ...
        def validation_attack(input, u_embedded, noise, decoder, indices_decoder, type="default"):
            ...
            # print(f"[DEBUG] u_embedded: {u_embedded.shape}, noised: {noised.shape}")
        ...
        with torch.no_grad():
            for step, (images, mask) in enumerate(iterator):
                ...
                val_uniface_er_all = validation_attack(input, u_embedded, uniface_swap, self.decoder_t, indices_decoder_t,type="all")
                val_uniface_er_df = validation_attack(input, u_embedded, uniface_swap, self.decoder_d, indices_decoder_d,type="deepfake")

                val_cscs_er_all = validation_attack(input, u_embedded, cscs, self.decoder_t, indices_decoder_t,type="all")
                val_cscs_er_df = validation_attack(input, u_embedded, cscs, self.decoder_d, indices_decoder_d,type="deepfake")

                val_fsrt_er_all = validation_attack(input, u_embedded, fsrt, self.decoder_t, indices_decoder_t,type="all")
                val_fsrt_er_df = validation_attack(input, u_embedded, fsrt, self.decoder_d, indices_decoder_d,type="deepfake")
                ...
                metrics["val_uniface_er_all"].append(decoded_message_error_rate_batch(val_uniface_er_all, watermark).detach().cpu())
                metrics["val_uniface_er_df"].append(decoded_message_error_rate_batch(val_uniface_er_df, watermark).detach().cpu())
                metrics["val_cscs_er_all"].append(decoded_message_error_rate_batch(val_cscs_er_all, watermark).detach().cpu())
                metrics["val_cscs_er_df"].append(decoded_message_error_rate_batch(val_cscs_er_df, watermark).detach().cpu())

                metrics["val_fsrt_er_all"].append(decoded_message_error_rate_batch(val_fsrt_er_all, watermark).detach().cpu())
                metrics["val_fsrt_er_df"].append(decoded_message_error_rate_batch(val_fsrt_er_df, watermark).detach().cpu())
                ...
                data_robust = [
                    ...
                    ["UniFace", np.mean(metrics["val_uniface_er_all"]), "-", np.mean(metrics["val_uniface_er_df"])],
...
if __name__ == "__main__":
    ...
    model.load_state_dict(
        torch.load(
            "/root/autodl-tmp/code_bk/exp_highpass/2025.07.18-09.52.32/model_state_5.pth",
            map_location="cuda:0",
        ),
        strict=False
    )
