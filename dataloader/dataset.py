# import os
# from PIL import Image
# from torchvision import transforms
# from torch.utils.data import Dataset
# import torch
# import cv2
# from config import training_config as cfg


# class attrsImgDataset(Dataset):
#     def __init__(self, path, image_size, attr_path="celeba"):
#         super(attrsImgDataset, self).__init__()
#         self.image_size = image_size
#         self.image_dir = path
#         if attr_path[0 : len("celebahq")] != "celebahq":
#             self.attr_path = "/root/autodl-tmp/code_bk/network/noise/stargan/list_attr_celeba.txt"
#         else:
#             self.attr_path = "/root/autodl-tmp/code_bk/network/noise/stargan/CelebAMask-HQ-attribute-anno.txt"

#         self.selected_attrs = [
#             "Black_Hair",
#             "Blond_Hair",
#             "Brown_Hair",
#             "Male",
#             "Young",
#         ]
#         self.list = []
#         self.attr2idx = {}
#         self.idx2attr = {}
#         self.transform = transforms.Compose(
#             [                                                
#                 transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5]),
#             ]
#         )
#         self.preprocess()

#     def preprocess(self):
#         """Preprocess the CelebA attribute file."""
#         lines = [line.rstrip() for line in open(self.attr_path, "r")]
#         all_attr_names = lines[1].split()
#         for i, attr_name in enumerate(all_attr_names):
#             self.attr2idx[attr_name] = i
#             self.idx2attr[i] = attr_name

#         lines = lines[2:]

#         for i, line in enumerate(lines):
#             split = line.split()
#             basename = os.path.basename(split[0])
#             filename = os.path.splitext(basename)[0]
#             values = split[1:]

#             label = []
#             for attr_name in self.selected_attrs:
#                 idx = self.attr2idx[attr_name]
#                 label.append(values[idx] == "1")

#             if os.path.exists(
#                 os.path.join(self.image_dir, str(filename).zfill(5) + ".png")
#             ):
#                 self.list.append([str(filename).zfill(5) + ".png", label])


#     def __getitem__(self, index):
#         filename, label = self.list[index]
#         image = cv2.imread(os.path.join(self.image_dir, filename))

#         if image is not None:
#             try:
#                 image = cv2.resize(image, (cfg.image_size, cfg.image_size))
#             except cv2.error as e:
#                 height, width = image.shape[:2]
#                 print(f"Error resizing image {os.path.join(self.image_dir, filename)} (size: {width}x{height}): {e}")
#         else:
#             print(f"image is none: {os.path.join(self.image_dir, filename)}")

#         image = cv2.cvtColor(image, cv2.COLOR_BGR2YUV)
#         image = torch.FloatTensor(image / 127.5 - 1.0)
#         if image is not None:
#             label_tensor = torch.FloatTensor(label)
#         return image.permute(2, 0, 1), label_tensor


#     def __len__(self):
#         return len(self.list)





import os
from torch.utils.data import Dataset
import torch
import cv2
from config import training_config as cfg


class attrsImgDataset(Dataset):
    def __init__(self, path, image_size, attr_path="celeba"):
        super().__init__()

        self.image_size = image_size
        self.image_dir = path

        if not attr_path.startswith("celebahq"):
            self.attr_path = (
                "/root/autodl-tmp/code_bk/network/noise/"
                "stargan/list_attr_celeba.txt"
            )
        else:
            self.attr_path = (
                "/root/autodl-tmp/code_bk/network/noise/"
                "stargan/CelebAMask-HQ-attribute-anno.txt"
            )

        self.selected_attrs = [
            "Black_Hair",
            "Blond_Hair",
            "Brown_Hair",
            "Male",
            "Young",
        ]

        self.list = []
        self.attr2idx = {}
        self.idx2attr = {}

        self.preprocess()

    def preprocess(self):
        """读取 CelebA-HQ 属性文件并建立图像列表。"""

        with open(self.attr_path, "r") as file:
            lines = [line.rstrip() for line in file]

        all_attr_names = lines[1].split()

        for i, attr_name in enumerate(all_attr_names):
            self.attr2idx[attr_name] = i
            self.idx2attr[i] = attr_name

        for line in lines[2:]:
            split = line.split()

            basename = os.path.basename(split[0])
            filename = os.path.splitext(basename)[0]

            # 完整数据集使用 00000.png 形式，而部分 small 数据集使用
            # 0.png/1015.png 形式。优先保持原来的五位命名，同时兼容
            # 未补零的文件名，并记录磁盘上真实存在的名称。
            candidate_filenames = [
                str(filename).zfill(5) + ".png",
                str(filename) + ".png",
            ]

            values = split[1:]

            label = []
            for attr_name in self.selected_attrs:
                idx = self.attr2idx[attr_name]
                label.append(values[idx] == "1")

            target_filename = None
            for candidate_filename in candidate_filenames:
                image_path = os.path.join(
                    self.image_dir,
                    candidate_filename,
                )
                if os.path.exists(image_path):
                    target_filename = candidate_filename
                    break

            if target_filename is not None:
                self.list.append([target_filename, label])

    def __getitem__(self, index):
        filename, label = self.list[index]
        image_path = os.path.join(self.image_dir, filename)

        # OpenCV 默认按 BGR 顺序读取
        image = cv2.imread(image_path)

        if image is None:
            raise FileNotFoundError(
                f"无法读取图像：{image_path}"
            )

        image = cv2.resize(
            image,
            (self.image_size, self.image_size)
        )

        # BGR 转成 RGB
        image = cv2.cvtColor(
            image,
            cv2.COLOR_BGR2RGB
        )

        # [0, 255] 转为 [-1, 1]
        image = image.astype("float32") / 127.5 - 1.0

        # [H, W, C] 转为 [C, H, W]
        image = torch.from_numpy(image).permute(2, 0, 1)

        label_tensor = torch.tensor(
            label,
            dtype=torch.float32
        )

        return image, label_tensor

    def __len__(self):
        return len(self.list)
