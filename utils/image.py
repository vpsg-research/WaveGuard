...
def save_images(saved_all, epoch, folder, resize_to=None):
    ...
    stacked_images = torch.cat(
        [
            images.unsqueeze(0),
            watermarked_images.unsqueeze(0),
            noised_images.unsqueeze(0),
            diff_w2co.unsqueeze(0),
            diff_w2no.unsqueeze(0),
        ],
        dim=0,
    )
    shape = stacked_images.shape
    stacked_images = stacked_images.permute(0, 3, 1, 4, 2).reshape(
        shape[3] * shape[0], shape[4] * shape[1], shape[2]
    )
    # Add 0.5 after unnormalizing to [0, 255] to round to nearest integer
    stacked_images = (
        stacked_images.mul(255).add_(0.5).clamp_(0, 255).to("cpu", torch.uint8).numpy()
    )
    filename = os.path.join(folder, "epoch-{}.png".format(epoch))

    saved_image = Image.fromarray(np.array(stacked_images, dtype=np.uint8)).convert(
        "RGB"
    )
