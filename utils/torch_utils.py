...
def normalize(tensor):
    # 1. 手动将 0-255 缩放到 0-1
    # 假设 tensor 是 yuv_denormalize 出来的 [0, 255] 范围
    tensor_0_1 = tensor / 255.0
    
    # 2. 执行分布归一化，将 [0, 1] 变为 [-1, 1]
    normalizer = transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])
    return normalizer(tensor_0_1)
...
# def decoded_message_error_rate_batch(gt, pred, mask=None, threshold=0.0):
#     gt_b = torch.where(gt > threshold, torch.tensor(1.0, device=gt.device), torch.tensor(-1.0, device=gt.device))
#     pred_b = torch.where(pred > threshold, torch.tensor(1.0, device=pred.device), torch.tensor(-1.0, device=pred.device))
#     error_map = torch.abs(gt_b - pred_b) / 2.0
#     if mask is not None:
#         m_b = (mask > 0.5).float() 
#         active_errors = torch.sum(error_map * m_b)
#         total_active_bits = torch.sum(m_b)
#         if total_active_bits == 0:
#             return 0.0
#         return (active_errors / total_active_bits).item() * 100
#     else:
#         return torch.mean(error_map).item() * 100
...
def decoded_message_error_rate_batch(gt, pred, mask=None, threshold=0.0):
    ...
    if mask is not None:
        ...
        return active_errors / total_active_bits
    else:
        return torch.mean(error_map)


# def decoded_message_error_rate_batch(gt, pred, mask=None, threshold=0.5):
#     gt_b = (gt > threshold).float()
#     pred_b = (pred > threshold).float()

#     error_map = (gt_b != pred_b).float()

#     if mask is not None:
#         m_b = (mask > 0.5).float().to(error_map.device)
#         m_b = torch.broadcast_to(m_b, error_map.shape)

#         total_active_bits = m_b.sum()
#         if total_active_bits.item() == 0:
#             return torch.tensor(0.0, device=gt.device)

#         return (error_map * m_b).sum() / total_active_bits
