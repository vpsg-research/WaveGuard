...
class Decoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # 解码器插值模式与编码器保持一致：水印长度大于特征图时用 bilinear，否则 nearest
        target_h, target_w = message.shape[2], message.shape[3]
        interp_mode = "bilinear" if cfg.message_length > max(target_h, target_w) else "nearest"
        message = F.interpolate(message, size=(cfg.message_length, cfg.message_length), mode=interp_mode)
