...
class CBAMAttention(nn.Module):
    def __init__(self, in_channels, out_channels, reduction=8):
        ...
        self.ca = ChannelAttention(
            in_channels=in_channels, out_channels=out_channels, reduction=reduction
        )
    ...
    def __init__(self, in_channels, out_channels, reduction, stride, attention=None):
        ...
        if attention == "se":
            ...
            self.attention = SEAttention(
                in_channels=out_channels, out_channels=out_channels, reduction=reduction
            )
        elif attention == "cbam":
            print("CBAMAttention")
...
class ResBlock(nn.Module):
    def __init__(
        ...
        layers = (
            [
                eval(block_type)(
                    in_channels, out_channels, reduction, stride, attention=attention
                )
            ]
            if blocks != 0
            else []
        ...
        for _ in range(blocks - 1):
            layer = eval(block_type)(
                out_channels, out_channels, reduction, 1, attention=attention
            )
            layers.append(layer)
